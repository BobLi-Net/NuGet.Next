    ...
    async function parseNuget(file: File) {
        ...
        try {
            ...
            xmlDoc.querySelectorAll('metadata').forEach((metadata) => {
                ...
                // Dependencies
                const dependencies: { id: string | null; version: string | null; exclude: string | null; }[] = [];
            ...
            >
                <Descriptions bordered column={1}>
                    ...
                    <Descriptions.Item label="许可证 URL">
                        <a href={item.licenseUrl} target="_blank" rel="noreferrer">{item.licenseUrl}</a>
                    </Descriptions.Item>
                    <Descriptions.Item label="项目 URL">
                        <a href={item.projectUrl} target="_blank" rel="noreferrer">{item.projectUrl}</a>
                    </Descriptions.Item>
                    <Descriptions.Item label="依赖项">
                        {item.dependencies && item.dependencies.map((dep: any, index: number) => (
                    ...
                    <Descriptions.Item label="框架引用">
                        {item.frameworkReferences && item.frameworkReferences.map((ref: any, index: number) => (
                            <div key={index} style={{ marginLeft: '20px' }}>
                                <div><strong>目标框架:</strong> {ref.targetFramework}</div>
                                <div><strong>引用:</strong> {ref.references && ref.references.map((reference: any, refIndex: number) => (
                                    <div key={refIndex} style={{ marginLeft: '20px' }}>
                                        <div><strong>名称:</strong> {reference.name}</div>
                                    </div>
                                ))}</div>
                            </div>
                        ))}
    ...
    return (
        <Flexbox
            ...
            <span style={{
                ...
                要了解有关出色软件包的详细信息，请参阅 <a href="https://learn.microsoft.com/zh-cn/nuget/create-packages/package-authoring-best-practices" target="_blank" rel="noreferrer">NuGet 文档</a>
            </span>
