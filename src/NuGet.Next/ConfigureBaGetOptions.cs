...
namespace NuGet.Next
{
    ...
    public class ConfigureBaGetOptions
        ...
        , IValidateOptions<NuGetNextOptions>
    {
        ...
        public ValidateOptionsResult Validate(string name, NuGetNextOptions options)
        {
            var failures = new List<string>();

            if (options.Database == null) failures.Add($"The '{nameof(NuGetNextOptions.Database)}' config is required");
            if (options.Mirror == null) failures.Add($"The '{nameof(NuGetNextOptions.Mirror)}' config is required");
            if (options.Search == null) failures.Add($"The '{nameof(NuGetNextOptions.Search)}' config is required");
            if (options.Storage == null) failures.Add($"The '{nameof(NuGetNextOptions.Storage)}' config is required");

            if (!ValidDatabaseTypes.Contains(options.Database?.Type))
            {
                failures.Add(
                    $"The '{nameof(NuGetNextOptions.Database)}:{nameof(DatabaseOptions.Type)}' config is invalid. " +
            ...
            {
                failures.Add(
                    $"The '{nameof(NuGetNextOptions.Storage)}:{nameof(StorageOptions.Type)}' config is invalid. " +
            ...
            {
                failures.Add(
                    $"The '{nameof(NuGetNextOptions.Search)}:{nameof(SearchOptions.Type)}' config is invalid. " +
