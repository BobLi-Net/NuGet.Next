...
{
    public const string PackageDownloadsV1Url =
        "https://nugetprod0.blob.core.windows.net/ng-search-data/downloads.v1.json";
            ...
            while (jsonReader.Read())
                try
                {
                    if (jsonReader.TokenType == JsonToken.StartArray)
                    {
                        ...
                        if (record.Count() == 2 && record[1].Type != JTokenType.Array) continue;

                        if (!results.ContainsKey(id)) results.Add(id, new Dictionary<string, long>());

                        foreach (var token in record)
                            if (token != null && token.Count() == 2)
                            {
                                var version = string.Intern(NuGetVersion.Parse(token[0].ToString()).ToNormalizedString()
                                    .ToLowerInvariant());
