...
public class V2UpstreamClient : IUpstreamClient, IDisposable
{
        ...
        ILogger logger)
    {
        ...
        if (options.Mirror?.PackageSource?.AbsolutePath == null)
        ...
        _repository = Repository.Factory.GetCoreV2(new PackageSource(options.Mirror.PackageSource.AbsoluteUri));
    }
    ...
    public async Task<IReadOnlyList<NuGetVersion>> ListPackageVersionsAsync(string id,
