...
{
    public Claim[] GetClaimsFromToken(User user)
    {
        var claims = new[]
        {
            ...
            new Claim("User", JsonSerializer.Serialize(user, new JsonSerializerOptions
            {
    ...
    public (string? id, string? role, string? fullName) GetUserFromToken(string token)
    {
        ...
        var fullName = jsonToken?.Claims.FirstOrDefault(x => x.Type == ClaimTypes.NameIdentifier)?.Value;
